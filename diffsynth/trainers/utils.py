...
class VideoDataset(torch.utils.data.Dataset):
    ...
    def load_video(self, file_path):
        ...
        num_frames = self.get_num_frames(vr)
        all_frames = vr.get_batch(range(num_frames)).asnumpy().astype(np.uint8)  # shape (N, H, W, C)
...
def launch_training_task(
    ...
    num_workers = 0
    dataloader = torch.utils.data.DataLoader(dataset, shuffle=True, collate_fn=lambda x: x[0], num_workers=num_workers, batch_size=1)
    # dataloader = torch.utils.data.DataLoader(dataset, shuffle=True, num_workers=num_workers, batch_size=2)
