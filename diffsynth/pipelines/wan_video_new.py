...
class WanVideoPipeline(BasePipeline):
    ...
    def training_loss(self, **inputs):
        ...
        # ========== TFM training with improved loss ==========
        inputs["latents"], training_target = self.scheduler.training_target(inputs["input_latents"], inputs["noise"], timestep)
        noise_pred = self.model_fn(**inputs, timestep=timestep)
        loss = self.scheduler.calculate_loss(inputs["input_latents"], inputs["noise"], inputs["latents"], noise_pred, training_target)
    ...
    def __call__(
        ...
        for progress_id, timestep in enumerate(progress_bar_cmd(self.scheduler.timesteps)):
            ...
            # Scheduler
            inputs_shared["latents"], sigma = self.scheduler.step(noise_pred, self.scheduler.timesteps[progress_id], inputs_shared["latents"], use_new_timestep=use_new_timestep and (progress_id > 0))
